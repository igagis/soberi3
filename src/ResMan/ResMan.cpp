...
const char* DResDescrTag = "ResourcesDescription";
...
Resource::Resource(){
	M_RESOURCE_LOG(<< "Resource::Resource(): invoked" << std::endl)
}
...
Resource::~Resource()throw(){
	...
	if(Ref<ResMapRC> resMap = this->rm){ //if Resource map still exists
		M_RESOURCE_LOG(<<"Resource::~Resource(): removing resource"<<std::endl)
		...
		M_RESOURCE_LOG(<<"Resource::~Resource(): resource removed"<<std::endl)
	}else{
		M_RESOURCE_LOG(<<"Resource::~Resource(): resource map is no longer existent"<<std::endl)
	}
}
