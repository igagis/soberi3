...
void TexFont::Load(File& fi, const wchar_t* chars, unsigned size, unsigned outline){
...
//	TRACE(<< "TexFont::Load(): FreeType library inited" << std::endl)
	...
	class FreeTypeFaceWrapper{
		...
		Array<u8> fontFile;//the buffer should be alive as long as the Face is alive!!!
	public:
		FreeTypeFaceWrapper(FT_Library& lib, File& fi){
...
//	TRACE(<< "TexFont::Load(): FreeType font face loaded" << std::endl)
	...
	struct sf{
		...
		inline static unsigned DXGap(){
			return 1;
		}
		inline static unsigned DYGap(){
			return 1;
		}
	};
	...
	unsigned texWidth;
	{
		...
		texWidth = std::max(unsigned(128), sf::FindNextPowOf2((numChars / 8) * size)); //divide by 8 is a good guess that all font characters will be placed in 8 rows on texture
		texWidth = std::min(std::min(maxTexSize, unsigned(1024)), texWidth);//clamp width to min of max texture size and 1024
	}

	unsigned curTexHeight = sf::FindNextPowOf2(size);//first guess of texture height
	unsigned curX = sf::DXGap();
	unsigned curY = sf::DYGap();
...
//		TRACE(<< "TexFont::Load(): glyph image created" << std::endl)
		...
		if(texImg.Width() < curX + im.Width() + sf::DXGap()){
			curX = sf::DXGap();
			curY += maxHeightInRow + sf::DYGap();
			maxHeightInRow = 0;
		}

		if(texImg.Height() < curY + im.Height() + sf::DYGap()){
			//grow texture size
			unsigned newHeight = sf::FindNextPowOf2(curY + im.Height() + sf::DYGap());
		...
		//record glyph information
		{
			...
			g.verts[0] = (Vec2f(float(m->horiBearingX), float(m->horiBearingY - m->height)) / (64.0f)) + Vec2f(-int(outline), -int(outline));
			g.verts[1] = (Vec2f(float(m->horiBearingX), float(m->horiBearingY)) / (64.0f)) + Vec2f(-int(outline), int(outline));
			g.verts[2] = (Vec2f(float(m->horiBearingX + m->width), float(m->horiBearingY)) / (64.0f)) + Vec2f(int(outline), int(outline));
			g.verts[3] = (Vec2f(float(m->horiBearingX + m->width), float(m->horiBearingY - m->height)) / (64.0f)) + Vec2f(int(outline), -int(outline));

			g.texCoords[0] = Vec2f(float(curX), float(curY + im.Height()));
			g.texCoords[1] = Vec2f(float(curX), float(curY));
			g.texCoords[2] = Vec2f(float(curX + im.Width()), float(curY));
			g.texCoords[3] = Vec2f(float(curX + im.Width()), float(curY + im.Height()));
...
//	TRACE(<< "TexFont::Load(): for loop finished" << std::endl)
	...
	if(curY == 0){
		...
		//resize
		Image copy(texImg);
		texImg.Init(sf::FindNextPowOf2(curX), copy.Height(), copy.Type());
		texImg.Clear();
		texImg.Blit(0, 0, copy);
	}
...
#ifdef DEBUG
void TexFont::RenderTex(const tride::Matr4f& matrix)const{
